	<body >
	<map name="menuMap">
		<area shape="poly" title="" coords="903,66, 938,140, 802,204, 766,131" href='#'/>
		<area shape="poly" title="" coords="766,132, 802,204, 666,270, 629,197" href='#'/>
		<area shape="poly" title="" coords="629,198, 665,270, 529,336, 492,263" href="#"/>
		<area shape="poly" title="" coords="492,264, 528,336, 391,402, 354,329" href="#"/>
		<area shape="poly" title="" coords="354,330, 390,402, 253,469, 217,395" href="#"/>
		<area shape="poly" title="" coords="200,369, 20,456, 67,554, 248,467" href="#">
	</map>

	<div class="subheader">
		<div class="temp" style="display:block">
			<div class="brokennavcontainer">
				<div class="brokennavloc" style="background-image:url(<?php echo $images_path; ?>lang/es/brokennav.gif);">
					<div class="shatter2"></div>
					<div class="brokennav">
						<img src="<?php echo $images_path; ?>layout/pixel.gif" usemap="#menuMap"/>
						<div class="gratz">
						    <p class="lead"><strong>Bien hecho</strong>, la rompiste.</p> 
								<p>Esta p&aacute;gina no existe o hubo un error horrible.</p>
						</div>
					</div>
					<div class="shatter1"></div>
				</div>						
			</div>					
		</div>
	</div>